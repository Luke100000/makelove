...
def common_prefix(paths):
    ...
    return max(
        paths[0][:i]
        for i in range(len(min(paths, key=len)))
        if all(name.startswith(paths[0][:i]) for name in paths)
    )
...
def build_windows(config, version, target, target_directory, love_file_path):
    ...
    def src(path):
        return os.path.join(love_binaries, path)

    def dest(path):
        return os.path.join(temp_archive_dir, path)

    def copy(path):
        return shutil.copyfile(src(path), dest(path))
